...
- [Node.js Runtime \& Performance Playbook](#nodejs-runtime--performance-playbook)
  ...
  - [The diagnostics toolbox](#the-diagnostics-toolbox)
  - [Scaling the process model](#scaling-the-process-model)
...
## The diagnostics toolbox

"What I want available" only counts when the kit is concrete and already in muscle memory:

- **`node --inspect` + Chrome DevTools** — heap snapshots and CPU profiles while developing
- **`node --prof` / `--cpu-prof`** — production-shaped profiling when attaching a debugger is not an option
- **`perf_hooks`** — `monitorEventLoopDelay()` and `PerformanceObserver` for in-process latency signals
- **`diagnostics_channel`** — zero-cost instrumentation hooks that stay silent until someone subscribes
- **`v8.writeHeapSnapshot()`** — wired to a signal so a live process can dump its heap on demand
- **clinic.js** (doctor / flame) — guided investigation when I don't yet know where to look

The workflow should be rehearsed before the incident.  
An outage is the wrong moment to learn which flag produces which file.

---

## Scaling the process model

Node gives you one event loop per process. Scaling past it is a ladder, not a single trick.

- **`worker_threads`** for CPU-bound work inside a service — behind a piscina-style pool, not a thread per task
- prefer **transferable** objects over structured cloning when moving large payloads to workers
- **cluster / multiple processes** for scaling request throughput on one box
- **horizontal replicas** as the real production answer for capacity
- the event loop stays reserved for short IO coordination work

Don't reach for threads until a profile shows CPU-bound work.  
Most latency problems are IO-shaped, and threads make those worse, not better.
